...
ADMIN_TOKEN=`curl -k --header "Content-Type: application/json" --request POST --data "{\"username\": \"admin\",\"email\": \"$EMAIL_ADDRESS\", \"password\": \"$PASSWORD\"}"   --url https://$HOSTNAME/admin/initial-admin-user?token=$INITIAL_TOKEN`
